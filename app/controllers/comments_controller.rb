class CommentsController < ApplicationController
  def index
    ...
    @comments = @q.result(:distinct => true).includes(:commentor, :result).page(params[:page]).per(10)
  ...
  def create_row_from_result
    ...
    if @comment.valid?
      ...
      redirect_to("/results/#{@comment.result_id}", notice: "Comment created successfully.")
    else
      render("comment_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_result
    @comment = Comment.find(params.fetch("id_to_remove"))

    @comment.destroy

    redirect_to("/results/#{@comment.result_id}", notice: "Comment deleted successfully.")
  end
